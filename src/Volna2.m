function [map, isPath, lenPath] = Volna2(map, coord, acsessCoordMass, isPath, lenPath,depthRecursion)
...
if(~isPath) %если путь найден на этом этапе - выйти из функции
    ...
    if(~(isempty(newCoord))) % если есть, куда идти
        ...
        acsessCoordMass = sortCoord(coordSource,coordDist,acsessCoordMass); % упорядочиваем! (вся соль)
...
%            acsessCoordMass(1 : min(sizeAcsessCoord(1),4) ,:)% просто вывод, потом удалить
        ...
        while(nAcsessCoord>0) % идём во все доступные точки
            if(~isPath) %если путь найден на нижнем этапе - выйти из функции
                ...
                pause(speed)
                [map, isPath, lenPath] = Volna2(map,nextCoord,acsessCoordMass, isPath, lenPath, depthRecursion+1);
            else
                break;
            end
        end
    %else
        %acsessCoordMass(1,:)=[]; % удалить точку, у которой некуда идти дальше
    end
end
